...
use App\Modules\Products\Models\{Product,ProductAttribute,ProductAttributeValue,ProductCategory,ProductCollection,ProductCollectionLine,ProductImage,ProductSetting,ProductType,ProductVariant};
...
class ProductsController extends Controller {
    ...
    public function store(StoreProductRequest $r): RedirectResponse { $this->validateImageUploads($r); $product=DB::connection('tenant')->transaction(fn()=> $this->saveProduct(new Product,$r->validated())); $this->storeFormImages($r,$product); return redirect()->route('products.show',$product)->with('success','Producto creado.'); }
    public function show(Product $product): View { $product->load(['type','category.parent','collection','line','generalImages','variants.images','variants.attributeValues.attribute']); return view('tenant.products.show',compact('product')); }
    public function edit(Product $product): View { return view('tenant.products.form',$this->formData(['product'=>$product->load('generalImages','variants.images','variants.attributeValues')])); }
    public function update(StoreProductRequest $r,Product $product): RedirectResponse { $this->validateImageUploads($r); DB::connection('tenant')->transaction(fn()=> $this->saveProduct($product,$r->validated())); $this->storeFormImages($r,$product); return redirect()->route('products.show',$product)->with('success','Producto actualizado.'); }
    ...
    private function validateImageUploads(Request $request): void { $request->validate(['images'=>['nullable','array'],'images.*'=>['file','mimes:jpg,jpeg,png,webp','max:5120'],'variant_images'=>['nullable','array'],'variant_images.*'=>['nullable','array'],'variant_images.*.*'=>['file','mimes:jpg,jpeg,png,webp','max:5120']]); }
    private function storeFormImages(Request $request, Product $product): void { foreach($request->file('images',[]) as $file)$this->storeImage($request,$product,null,$file); foreach($request->file('variant_images',[]) as $key=>$files){$variant=$product->variants()->find($key)?:$product->variants()->where('sku',$request->input("variants.$key.sku"))->first();if(!$variant)continue;foreach($files as $file)$this->storeImage($request,$product,$variant,$file);} }
    private function storeImage(Request $request, Product $product, ?ProductVariant $variant, $file): void { if(!$file->isValid())return;$account=$request->attributes->get('tenantAccount');$directory=$variant?"tenants/{$account->id}/products/{$product->id}/variants/{$variant->id}":"tenants/{$account->id}/products/{$product->id}/images";$images=$variant?$variant->ownImages():$product->generalImages();$path=$file->store($directory,'local');ProductImage::create(['product_id'=>$product->id,'product_variant_id'=>$variant?->id,'path'=>$path,'original_filename'=>$file->getClientOriginalName(),'is_primary'=>!$images->exists(),'sort_order'=>(int)$images->max('sort_order')+1]); }
