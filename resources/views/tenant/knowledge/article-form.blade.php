@php
    ...
    $version = $editableVersion ?? null;

    $selectedCategories = old(
        'category_ids',
        $editing
            ? $article->categories->pluck('id')->all()
            : []
    );

    $selectedAudience = old(
        'audience',
        $version?->audience ?? ['all']
    );
@endphp

<x-layouts.tenant
    :title="$editing ? 'Editar artículo' : 'Nuevo artículo'"
    :subtitle="$editing && $article->status === 'published'
        ? 'Se editará como nueva versión en borrador.'
        : 'Knowledge Center'"
>
    <div class="tr-card">
        <form
            method="POST"
            action="{{ $editing
                ? route('knowledge.articles.update', $article)
                : route('knowledge.articles.store') }}"
        >
            @csrf
            ...
            <div class="row g-3">
                    ...
                    @foreach ([
                        ...
                        <div class="form-check form-check-inline">
                            <input
                                ...
                                value="{{ $code }}"
                                id="audience-{{ $code }}"
                                @checked(in_array($code, $selectedAudience))
                            >
                ...
                <div class="col-12">
                    ...
                    <textarea
                        ...
                        name="content"
                        required
                    >{{ old('content', $version?->content ?? '') }}</textarea>
