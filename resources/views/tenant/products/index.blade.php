<x-layouts.tenant title="Catálogo" subtitle="Productos y variantes de la cuenta activa">
    ...
    <form class="tr-card mb-3" method="GET">
        <div class="row g-2">
            <div class="col-md-4">
                <input class="form-control" name="search" value="{{ request('search') }}" placeholder="Nombre, código catálogo o SKU">
            </div>
            <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <select class="form-select" name="product_collection_id">
                        ...
                        @foreach ($collections as $collection)
                            <option value="{{ $collection->id }}" @selected((int) request('product_collection_id') === $collection->id)>
                                {{ $collection->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-2">
                    <select class="form-select" name="product_collection_line_id">
                        ...
                        @foreach ($lines as $line)
                            <option value="{{ $line->id }}" @selected((int) request('product_collection_line_id') === $line->id)>
                                {{ $line->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-2">
                <select class="form-select" name="product_type_id">
                    ...
                    @foreach ($productTypes as $type)
                        <option value="{{ $type->id }}" @selected((int) request('product_type_id') === $type->id)>
                            {{ $type->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <div class="col-md-2">
                    <select class="form-select" name="attribute_{{ $attribute->id }}">
                        ...
                        @foreach ($attribute->values as $value)
                            <option value="{{ $value->id }}" @selected((int) request('attribute_' . $attribute->id) === $value->id)>
                                {{ $value->value }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="col-12 d-flex flex-wrap gap-2">
                ...
                <a class="btn btn-outline-primary ms-md-auto" href="{{ route('products.imports.create') }}">
            ...
            <table class="table table-custom align-middle mb-0">
                ...
                <tbody>
                    ...
                    @empty
                        <tr>
                            <td colspan="11">
                                <div class="listing-empty">No hay productos todavía.</div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
    ...
    <form id="bulk-form" class="tr-card mt-3" method="POST" action="{{ route('products.bulk') }}">
        ...
        <div class="row g-2 align-items-end">
            <div class="col-md-3">
                <label class="form-label">Categoría masiva</label>
                <select class="form-select" name="category_id">
            ...
            @if ($settings->manages_collections)
                <div class="col-md-3">
                    <label class="form-label">Colección masiva</label>
                    <select class="form-select" name="product_collection_id">
                        <option value="">No cambiar</option>
                        @foreach ($collections as $collection)
            ...
            @if ($settings->manages_collection_lines)
                <div class="col-md-3">
                    <label class="form-label">Línea masiva</label>
                    <select class="form-select" name="product_collection_line_id">
                        <option value="">No cambiar</option>
                        @foreach ($lines as $line)
                            <option value="{{ $line->id }}">
                                {{ $line->collection->name ?? 'Colección' }} · {{ $line->name }}
                            </option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="col-md-2">
                <button class="btn btn-outline-primary">Aplicar a seleccionados</button>
            </div>
        </div>
    </form>
</x-layouts.tenant>
