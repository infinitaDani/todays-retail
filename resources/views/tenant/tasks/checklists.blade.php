<x-layouts.tenant
    title="Checklists"
    subtitle="Plantillas operativas asociadas a turnos"
>
    <div class="row g-3 mb-4">
        @foreach ([
            ['Total', $summary['total'], 'clipboard-list', 'primary'],
            ['Activos', $summary['active'], 'check-circle-2', 'success'],
            ['Inactivos', $summary['inactive'], 'pause-circle', 'warning'],
            ['Con historial', $summary['used'], 'history', 'primary'],
                ...
                >
                    ...
                    @foreach ($shifts as $shift)
                        <option
                            value="{{ $shift->id }}"
                            @selected(
                                (string) request('shift_id') === (string) $shift->id
                            )
                        >
                            {{ $shift->name }}
                        </option>
                ...
                <tbody>
                    @forelse ($checklists as $checklist)
                        <tr>
                            ...
                            <td>
                                {{ $checklist->shifts->pluck('name')->join(', ') }}
                            </td>
                            ...
                            <td>
                                {{ $checklist->updated_at->format('d/m/Y') }}
                            </td>

                            <td>
                                <span
                                    class="badge badge-label badge-soft-{{ $checklist->status === 'active'
                                        ? 'success'
                                        : 'warning' }}"
                                >
